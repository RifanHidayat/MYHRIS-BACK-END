  ...
  if (cachedData) {
    ...
    if (cachedData) {
      ...
      for (let data of cachedData) {
        ...
        const jamIstirahatKeluar = new Date(
          `${dateNow}T${waktuIstirahatkeluar}`
        ); //jam Keluar
        ...
        jamIstirahatMasuk.setMinutes(
          jamIstirahatMasuk.getMinutes() + parseInt(startIstirahatMasuk)
        );
        jamIstirahatKeluar.setMinutes(
          jamIstirahatKeluar.getMinutes() + parseInt(startIstirahatKeluar)
        );
        ...
        if (
          (jamMasuk.getMinutes() == jamSekarang.getMinutes() &&
            jamMasuk.getHours() == jamSekarang.getHours()) ||
          (jamKeluar.getMinutes() == jamSekarang.getMinutes() &&
            jamKeluar.getHours() == jamSekarang.getHours()) ||
          (jamIstirahatKeluar.getMinutes() == jamSekarang.getMinutes() &&
            jamIstirahatKeluar.getHours() == jamSekarang.getHours()) ||
          (jamIstirahatMasuk.getMinutes() == jamSekarang.getMinutes() &&
            jamIstirahatMasuk.getHours() == jamSekarang.getHours() &&
            data.status == "absensi")
        ) {
          ...
          connection.connect((err) => {
            ...
            connection.beginTransaction((err) => {
              ...
              connection.query(
                `SELECT * FROM ${namaDatabaseDynamic}.attendance WHERE em_id='${data.em_id}' AND atten_date='${dateNow}' ORDER BY id DESC`,
                (err, results) => {
                  if (err) {
                  ...
                  connection.query(
                    `SELECT * FROM ${namaDatabaseDynamic}.emp_leave WHERE date_selected LIKE '%${dateNow}%' AND (ajuan='1' OR ajuan='2' OR ajuan='3')`,
                    (err, sakit) => {
                      if (err) {
                        console.error("Error executing SELECT statement:", err);
                        connection.rollback(() => {
                      ...
                      if (
                        ...
                        if (results.length >= 0) {
                          console.log(
                            "data status ",
                            data.full_name,
                            data.notif_absen_masuk
                          );
                          //munculin notif
                          var messageAbsenMasuk = `Reminder absen masuk`;
                          utility.pushNotifikasiApproval(
                            data.token,
                            "Absen Masuk",
                            messageAbsenMasuk,
                            "info absen",
                      ...
                      if (
                        ...
                        if (results.length > 0) {
                          if (
                            results[0].signout_time == "" ||
                            results[0].signout_time == "00:00:00" ||
                            results[0].signout_time == null
                          ) {
                            //munculin notif
                      ...
                      if (data.tipe_absen == "3" || data.tipe_absen == 3) {
                        if (
                          jamIstirahatKeluar.getMinutes() ==
                            jamSekarang.getMinutes() &&
                          jamIstirahatKeluar.getHours() ==
                            jamSekarang.getHours()
                        ) {
                          if (results.length == 1) {
                            if (
                              results[0].breakout_time == "" ||
                              results[0].breakout_time == "00:00:00" ||
                              results[0].breakout_time == null
                            ) {
                              //munculin notif
                              if (
                                data.notif_istirahat_keluar.includes("true")
                              ) {
                              } else {
                                if (
                                  data.token_notif == null ||
                        ...
                        if (
                          ...
                          if (results.length == 1) {
                            //munculin notif
                            if (
                              results[0].breakin_time == "" ||
                              results[0].breakin_time == "00:00:00" ||
                              results[0].breakin_time == null
                            ) {
                              //munculin notif
        ...
        if (
          jamlembur.getMinutes() == jamSekarang.getMinutes() &&
          jamlembur.getHours() == jamSekarang.getHours() &&
          data.status == "lembur"
        ) {
          console.log("lembur");
          ...
          connection.connect((err) => {
            ...
            connection.beginTransaction((err) => {
              ...
              connection.query(
                `SELECT * FROM ${namaDatabaseDynamic}.emp_labor WHERE em_id='${data.em_id}' AND atten_date='${dateNow}' AND ajuan='1' ORDER BY id DESC`,
                (err, results) => {
                  if (err) {
                    console.error("Error executing SELECT statement:", err);
                    connection.rollback(() => {
    ...
    connection.connect((err) => {
      ...
      connection.beginTransaction((err) => {
        ...
        connection.query(query, (err, results) => {
          ...
          connection.query(
            `SELECT em_ids FROM places_coordinate JOIN overtime ON places_coordinate.place  LIKE CONCAT('%',overtime.lokasi,'%')  `,
            (err, lokasi) => {
              if (err) {
                console.error("Error executing SELECT statement:", err);
                connection.rollback(() => {
  ...
  basicAuth({
    authorizer: (username, password) => {
      const userMatches = basicAuth.safeCompare(
        username,
        "aplikasioperasionalsiscom"
      );
      const passwordMatches = basicAuth.safeCompare(
        password,
        "siscom@ptshaninformasi#2022@"
      );
...
app.post("/upload_file_permintaan_kandidat", function (req, res) {
  ...
  sampleFile = req.files.sampleFile;
  uploadPath =
    __dirname + "/public/file_permintaan_kandidat/" + sampleFile.name;
