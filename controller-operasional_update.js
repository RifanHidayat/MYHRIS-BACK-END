...
module.exports = {
  ...
  async info_sisa_kontrak(req, res) {
    ...
    var query1 = `SELECT CURDATE(), TBL.em_id, ADDDATE(TBL.end_date, INTERVAL - ${reminder} DAY),
    ...
    console.log("info sisa konrak",query1)
  ...
  async whereOnce(req, res) {
    ...
    connection.connect((err) => {
      ...
      connection.beginTransaction((err) => {
        ...
        if (convert2 == 'assgin_leave') {
          query = `SELECT * FROM ${convert2} WHERE ${value}='${cari}' ORDER BY DESC dateyaer`;
...
//       req.end();
  ...
  validasiLogin(req, res) {
    ...
    poolDynamic.getConnection(function (err, connection) {
      ...
      if (err) console.log(err);
      connection.query(
        `SELECT em_id, full_name, em_email, des_id, dep_id, dep_group_id as dep_group, em_mobile as em_phone, em_birthday, em_blood_group, em_gender, em_image, em_joining_date, em_status, job_title as posisi, em_hak_akses, last_login, status as status_aktif, em_control, em_controlaccess as em_control_access, b.name AS emp_jobTitle,c.name AS emp_departmen,em_att_working as emp_att_working FROM employee a LEFT JOIN designation b ON a.des_id=b.id LEFT JOIN department c ON a.dep_id=c.id where em_email='${email}' AND em_password='${password}'`,
        function (error, results) {
          if (error) console.log(error);
          if (results.length == 0) {
            res.send({
  ...
  //     if (err) console.log(err);
  //     connection.query(
  //       `SELECT (SELECT beginday_payroll FROM payment_schedule WHERE is_default='Y' LIMIT 1) AS begin_payroll,(SELECT name FROM sysdata WHERE id='18') as time_attendance,
  //       (SELECT endday_payroll FROM payment_schedule WHERE is_default='Y' LIMIT 1) AS end_payroll, branch.name AS branch_name, em_id, full_name, em_email, des_id, dep_id, dep_group_id as dep_group, em_mobile as em_phone, em_birthday, em_blood_group, em_gender, em_image, em_joining_date, em_status, job_title as posisi, em_hak_akses, last_login, status as status_aktif, em_controlaccess as em_control_access, b.name AS emp_jobTitle,c.name AS emp_departmen,em_att_working as emp_att_working FROM employee a LEFT JOIN designation b ON a.des_id=b.id LEFT JOIN department c ON a.dep_id=c.id JOIN branch ON branch.id=a.branch_id where a.em_id='${em_id}'`,
  //       function (error, results) {
  //         connection.release();
  //         if (error != null) console.log(error)
  //         if (results.length == 0) {
      ...
      connection.beginTransaction((err) => {
        ...
        connection.query(`SELECT * FROM employee WHERE em_id='${emId}'`, (err, results) => {
          ...
          if (employee[0].em_hak_akses == "" || employee[0].em_hak_akses == "0") {
            query = "SELECT * FROM employee JOIN branch ON employee.branch_id=branch.id WHERE branch.name NOT LIKE '%HIDDEN%' ORDER BY full_name ASC"
          } else {
  ...
  load_approve_info(req, res) {
    ...
    var query8 = `SELECT a.em_id, b.full_name FROM ${namaDatabaseDynamic}.emp_labor a JOIN ${database}_hrm.employee b JOIN  ${database}_hrm.branch ON b.branch_id=branch.id WHERE a.em_id=b.em_id AND b.em_report_to LIKE '%${em_id}%' AND (a.status='Pending' OR a.status='pending') AND a.ajuan='3' `;
    ...
    const configDynamic = {
      multipleStatements: true,
      host: '3.1.189.143',//myhris.siscom.id (ip local)
      user: 'pro',
      password: 'Siscom3519',
      database: `${namaDatabaseDynamic}`,
      timezone: "+00:00",
    ...
    poolDynamic.getConnection(function (err, connection) {
      if (err) {
        ...
        connection.query(
          `${query1};${query2};${query3};${query4};${query5};${query6};${query7};${query8};`,[em_id],
          function (error, results) {
            if (error != null) console.log(error)
            connection.release();
            res.send({
  ...
  var query6 = `SELECT a.em_id, b.full_name FROM ${namaDatabaseDynamic}.emp_claim a JOIN ${database}_hrm.employee b JOIN  ${database}_hrm.branch ON b.branch_id=branch.id  WHERE a.em_id=b.em_id AND (b.em_report_to LIKE '%${em_id}%' OR b.em_report2_to LIKE '%${em_id}%')  AND a.status IN ('Pending', 'Approve') `;
 // var query7 = `SELECT a.em_id, b.full_name FROM ${namaDatabaseDynamic}.emp_mobile_approval a JOIN ${database}_hrm.employee b WHERE a.em_id=b.em_id   AND created_date='${req.body.date}' AND a.approved_id IS NULL`;
  var query7=`SELECT designation.payroll_approval, a.em_id, b.full_name FROM ${namaDatabaseDynamic}.emp_mobile_approval a 
  JOIN ${database}_hrm.employee b JOIN ${database}_hrm.designation ON designation.id=b.des_id WHERE a.em_id=b.em_id 
  AND (a.approved_id IS NULL OR  a.approved_id ='') AND designation.payroll_approval  IN (?)`    
  var query8 = `SELECT a.em_id, b.full_name FROM ${namaDatabaseDynamic}.emp_labor a JOIN ${database}_hrm.employee b JOIN  ${database}_hrm.branch ON b.branch_id=branch.id
   WHERE a.em_id=b.em_id AND b.em_report_to LIKE '%${em_id}%' AND (a.status='Pending' OR a.status='pending') AND a.ajuan='3'`;
  ...
  spesifik_approval(req, res) {
    ...
    var query1 = `SELECT b.em_report_to as em_report_to,  b.em_report2_to as em_report2_to,   b.full_name, c.name as nama_tipe, c.category, a.* FROM ${namaDatabaseDynamic}.emp_leave a INNER JOIN ${database}_hrm.leave_types c ON a.typeid=c.id JOIN ${database}_hrm.employee b WHERE a.em_id=b.em_id AND  b.em_report_to LIKE '%${em_id}%' AND a.leave_status='Pending' AND a.ajuan IN ('2', '3')`;
    var query2 = `SELECT b.em_report_to as em_report_to,  b.em_report2_to as em_report2_to,   b.full_name, c.name as nama_tipe, c.category, a.* FROM ${namaDatabaseDynamic}.emp_leave a INNER JOIN ${database}_hrm.leave_types c ON a.typeid=c.id JOIN ${database}_hrm.employee b WHERE a.em_id=b.em_id AND b.em_report_to LIKE '%${em_id}%' AND a.leave_status='Pending' AND a.ajuan='1'`;
    var query3 = `SELECT b.em_report_to as em_report_to,  b.em_report2_to as em_report2_to,   b.full_name, a.* FROM ${namaDatabaseDynamic}.emp_labor a JOIN ${database}_hrm.employee b WHERE a.em_id=b.em_id AND b.em_report_to LIKE '%${em_id}%' AND a.status='Pending' AND a.ajuan='1'`;
    var query4 = `SELECT b.em_report_to as em_report_to,  b.em_report2_to as em_report2_to,   b.full_name, a.* FROM ${namaDatabaseDynamic}.emp_labor a JOIN ${database}_hrm.employee b WHERE a.em_id=b.em_id AND b.em_report_to LIKE '%${em_id}%' AND a.status='Pending' AND a.ajuan='2'`;
   
    var query5 = `SELECT b.em_report_to as em_report_to,  b.em_report2_to as em_report2_to,   b.full_name, a.* FROM ${namaDatabaseDynamic}.emp_leave a JOIN ${database}_hrm.employee b WHERE a.em_id=b.em_id AND b.em_report_to LIKE '%${em_id}%' AND a.leave_status='Pending' AND a.ajuan='4'`;
    
    var query6 = `SELECT b.em_report_to as em_report_to,  b.em_report2_to as em_report2_to,   b.full_name, c.name as nama_tipe, a.* FROM ${namaDatabaseDynamic}.emp_claim a INNER JOIN ${database}_hrm.cost c ON a.cost_id=c.id JOIN ${database}_hrm.employee b WHERE a.em_id=b.em_id AND b.em_report_to LIKE '%${em_id}%' AND a.status='Pending'`
    ...
    var query7 = `SELECT b.em_report_to as em_report_to,  b.em_report2_to as em_report2_to,   b.full_name, a.*,a.status as leave_status FROM ${namaDatabaseDynamic}.emp_labor a JOIN ${database}_hrm.employee b WHERE a.em_id=b.em_id AND a.status='Pending' AND a.ajuan='3'`;
  ...
  async ApprovalAbsensi(req,res){
    console.log("approval employee attendance")
    var database=req.query.database;
  ...
  spesifik_approval_multi(req, res) {
    console.log('-----spesifik approval multi approvedw----------')
    var database = req.query.database;
    var em_id = req.body.em_id;
    var url_data = req.body.name_data;
    var getbulan = req.body.bulan;
    ...
    var query3 = `SELECT b.em_report_to as em_report_to,  b.em_report2_to as em_report2_to,   b.full_name, a.* FROM ${namaDatabaseDynamic}.emp_labor a JOIN ${database}_hrm.employee b WHERE a.em_id=b.em_id AND (b.em_report_to LIKE '%${em_id}%' OR b.em_report2_to LIKE '%${em_id}%') AND a.status IN ('Pending', 'Approve') AND a.ajuan='1'`;
    ...
    var query8 = `SELECT b.em_report_to as em_report_to,  b.em_report2_to as em_report2_to,   b.full_name, a.*,a.status as leave_status FROM ${namaDatabaseDynamic}.emp_labor a JOIN ${database}_hrm.employee b WHERE a.em_id=b.em_id AND a.status='Pending' AND a.ajuan='3'`;
 ...
 WHERE emp.em_id = att.em_id
  ...
  load_laporan_pengajuan(req, res) {
    ...
    var query1_tidak_hadir = `SELECT a.*, b.full_name, b.job_title, count(*) as jumlah_pengajuan FROM ${namaDatabaseDynamic}.emp_leave a JOIN ${database}_hrm.employee b ON a.em_id=b.em_id WHERE month(a.atten_date)='${req.body.bulan}' AND year(a.atten_date)='${req.body.tahun}' AND a.ajuan IN ('2','3') GROUP BY b.full_name`;
    var query2_tidak_hadir = `SELECT a.*, b.full_name, b.job_title, count(*) as jumlah_pengajuan FROM ${namaDatabaseDynamic}.emp_leave a JOIN ${database}_hrm.employee b ON a.em_id=b.em_id WHERE month(a.atten_date)='${req.body.bulan}' AND year(a.atten_date)='${req.body.tahun}' AND b.dep_id='${status}' AND a.ajuan IN ('2','3') GROUP BY b.full_name`;
    var query1_cuti = `SELECT a.*, b.full_name, b.job_title, count(*) as jumlah_pengajuan FROM ${namaDatabaseDynamic}.emp_leave a JOIN ${database}_hrm.employee b ON a.em_id=b.em_id WHERE month(a.atten_date)='${req.body.bulan}' AND year(a.atten_date)='${req.body.tahun}' AND a.ajuan='1' GROUP BY b.full_name`;
    var query2_cuti = `SELECT a.*, b.full_name, b.job_title, count(*) as jumlah_pengajuan FROM ${namaDatabaseDynamic}.emp_leave a JOIN ${database}_hrm.employee b ON a.em_id=b.em_id WHERE month(a.atten_date)='${req.body.bulan}' AND year(a.atten_date)='${req.body.tahun}' AND b.dep_id='${status}' AND a.ajuan='1' GROUP BY b.full_name`;
    var query1_lembur = `SELECT a.*, b.full_name, b.job_title, count(*) as jumlah_pengajuan FROM ${namaDatabaseDynamic}.emp_labor a JOIN ${database}_hrm.employee b ON a.em_id=b.em_id WHERE month(a.atten_date)='${req.body.bulan}' AND year(a.atten_date)='${req.body.tahun}' AND a.ajuan='1' GROUP BY b.full_name`;
    var query2_lembur = `SELECT a.*, b.full_name, b.job_title, count(*) as jumlah_pengajuan FROM ${namaDatabaseDynamic}.emp_labor a JOIN ${database}_hrm.employee b ON a.em_id=b.em_id WHERE month(a.atten_date)='${req.body.bulan}' AND year(a.atten_date)='${req.body.tahun}' AND b.dep_id='${status}' AND a.ajuan='1' GROUP BY b.full_name`;
    var query1_tugas_luar = `SELECT a.*, b.full_name, b.job_title, count(*) as jumlah_pengajuan FROM ${namaDatabaseDynamic}.emp_labor a JOIN ${database}_hrm.employee b ON a.em_id=b.em_id WHERE month(a.atten_date)='${req.body.bulan}' AND year(a.atten_date)='${req.body.tahun}' AND a.ajuan='2' GROUP BY b.full_name`;
    var query2_tugas_luar = `SELECT a.*, b.full_name, b.job_title, count(*) as jumlah_pengajuan FROM ${namaDatabaseDynamic}.emp_labor a JOIN ${database}_hrm.employee b ON a.em_id=b.em_id WHERE month(a.atten_date)='${req.body.bulan}' AND year(a.atten_date)='${req.body.tahun}' AND b.dep_id='${status}' AND a.ajuan='2' GROUP BY b.full_name`;
    var query1_dinasluar = `SELECT a.*, b.full_name, b.job_title, count(*) as jumlah_pengajuan FROM ${namaDatabaseDynamic}.emp_leave a JOIN ${database}_hrm.employee b ON a.em_id=b.em_id WHERE month(a.atten_date)='${req.body.bulan}' AND year(a.atten_date)='${req.body.tahun}' AND a.ajuan='4' GROUP BY b.full_name`;
    var query2_dinasluar = `SELECT a.*, b.full_name, b.job_title, count(*) as jumlah_pengajuan FROM ${namaDatabaseDynamic}.emp_leave a JOIN ${database}_hrm.employee b ON a.em_id=b.em_id WHERE month(a.atten_date)='${req.body.bulan}' AND year(a.atten_date)='${req.body.tahun}' AND b.dep_id='${status}' AND a.ajuan='4' GROUP BY b.full_name`;
    var query1_klaim = `SELECT a.*, b.full_name, b.job_title, count(*) as jumlah_pengajuan FROM ${namaDatabaseDynamic}.emp_claim a JOIN ${database}_hrm.employee b ON a.em_id=b.em_id WHERE month(a.created_on)='${req.body.bulan}' AND year(a.created_on)='${req.body.tahun}' GROUP BY b.full_name`;
    var query2_klaim = `SELECT a.*, b.full_name, b.job_title, count(*) as jumlah_pengajuan FROM ${namaDatabaseDynamic}.emp_claim a JOIN ${database}_hrm.employee b ON a.em_id=b.em_id WHERE month(a.created_on)='${req.body.bulan}' AND year(a.created_on)='${req.body.tahun}' AND b.dep_id='${status}' GROUP BY b.full_name`;
  ...
  load_detail_laporan_pengajuan(req, res) {
    ...
    var query_klaim = `SELECT a.*, b.name, b.category FROM ${namaDatabaseDynamic}.emp_claim a JOIN ${database}_hrm.cost b ON a.cost_id=b.id WHERE a.em_id='${em_id}' ORDER BY a.id DESC`;
...
//   * FROM attendance WHERE CONCAT(atten_date, ' ', signin_time) >= '2023-10-02 05:00:00' AND CONCAT(atten_date, ' ', signin_time)<= '2023-10-03 09:00:00'
  ...
  potong_cuti(req, res) {
    ...
    var query1 = `SELECT terpakai FROM assign_leave WHERE em_id='${em_id}'`;
    ...
    poolDynamic.getConnection(function (err, connection) {
      ...
      connection.query(
        ...
        function (error, results) {
          ...
          connection.query(
            `UPDATE assign_leave SET terpakai='${hitung}' WHERE em_id='${em_id}'`,
  ...
  info_aktifitas_employee(req, res) {
    ...
    var query_izin = `SELECT COUNT(*) as jumlah_izin FROM ${namaDatabaseDynamic}.emp_leave WHERE em_id='${em_id}' AND ajuan='3'`;
    var query_sakit = `SELECT COUNT(*) as jumlah_sakit FROM ${namaDatabaseDynamic}.emp_leave WHERE em_id='${em_id}' AND ajuan='2'`;
    var query_cuti = `SELECT COUNT(*) as jumlah_cuti FROM ${namaDatabaseDynamic}.emp_leave WHERE em_id='${em_id}' AND ajuan='1'`;
  ...
  saveEmployeeAttendance(req, res) {
    ...
    var lokasiKeluar=req.bod.lokasi_keluar_id
          ...
          function (error, results) {
            ...
            if (results.length>0){
             ...
             ` SELECT nomor_ajuan FROM emp_labor WHERE ajuan='3' ORDER BY id DESC `,
              ...
              connection.query(
                `INSERT INTO emp_labor (nomor_ajuan,em_id,atten_date,dari_jam,sampai_jam,tgl_ajuan,status,status_transaksi,uraian,ajuan,em_delegation,req_file)
                 VALUES ('${nomorAjuan}','${em_id}','${date}','${checkin}','${checkout}','${tanggalAjuan}','${status}','1','${catatan}','3','','${nameFile}')`,
                function (error, results) {
                  if (error != null) console.log(error)
  ...
  getEmployeeAttendance(req, res) {
    ...
    poolDynamic.getConnection(function (err, connection) {
      if (err) {
        ...
        connection.query(
          `SELECT * FROM emp_labor WHERE ajuan='3' AND em_id='${em_id}' AND status_transaksi=1 ORDER BY id DESC`,
          function (error, results) {
            if (error != null) console.log(error)
            res.send({
              status: true,
