    ...
    try {
      ...
      connection.connect((err) => {
        ...
        connection.beginTransaction((err) => {
          ...
          connection.query(
            `SELECT nomor_ajuan FROM ${namaDatabaseDynamic}.emp_labor  WHERE nomor_ajuan LIKE  '%WH%' ORDER BY nomor_ajuan DESC LIMIT 1 `,
            (err, empLabor) => {
              if (err) {
              ...
              connection.query(
                ...
                VALUES ('${nomorAjuan}','${emId}','${time}',CURDATE(),'00:00','${date}','Pending','1','Pending','${uraian}','${emId}','4','${place}','${sigingLonglat}','${signingAddr}',CURDATE()) `,
                (err, results) => {
                  if (err) {
                    console.error("Error executing SELECT statement:", err);
                    connection.rollback(() => {
          ...
          connection.query(script, [bodyValue], (err) => {
            ...
            connection.query(
              ...
              (err) => {
                ...
                connection.query(
                  `SELECT * FROM ${namaDatabaseDynamic}.emp_labor WHERE ${nameWhere} = '${cariWhere}'`,
                  (err, pengajuan) => {
                    if (err) {
                      console.error("Error executing UPDATE statement:", err);
                      connection.rollback(() => {
                    ...
                    connection.query(
                      `SELECT * FROM employee WHERE em_id ='${pengajuan[0].em_id}'`,
                      (err, employee) => {
                        if (err) {
                          console.error(
                        ...
                        if (pola == "1" || pola == 1) {
                          connection.query(
                            `INSERT INTO  ${namaDatabaseDynamic}.attendance 
                                      ...
                                      if (listData[i] != "") {
                                        ...
                                        connection.query(
                                          ...
                                          (err, employee) => {
                                            ...
                                            connection.query(
                                              `INSERT INTO ${namaDatabaseDynamic}.notifikasi (em_id,title,deskripsi,url,atten_date,jam,status,view,em_id_pengajuan,idx) VALUES ('${employee[0].em_id}','${title}','${deskripsi}','${urlTransaksi}',CURDATE(),CURTIME(),1,0,'${emId}','${pengajuan[0].id}')`,
