...
module.exports = {
  async approval(req, res) {
    ...
    var konsekuensi =
      req.body.konsekuensi == "" ? "none" : req.body.konsekuensi;
    ...
    try {
      ...
      console.log(getData);
      const allemIdUser = req.body.em_id_surat;
      console.log(allemIdUser);
      const allfullName = req.body.full_name_surat;
      ...
      if (status == "") {
        ...
        if (tipeSuratAudit == "teguran_lisan") {
          surat = "teguran_lisan";
          const queryHapus = `DELETE FROM ${databaseMaster}.${surat} WHERE id IN (${idSuratAudit})`;
        ...
        if (konsekuensi === "teguran_lisan") {
          let insertIds = [];
          let insertFullName = [];
          for (var j = 0; j < allemIdUser.length; j++) {
            var emIdUser = allemIdUser[j];
            var fullNameUser = allfullName[j];
            ...
            insertIds.push(insertTeguran.insertId);
            insertFullName.push(fullNameUser);
          ...
          let auditSuratIdString = insertIds.join(",");
          let fullNameString = insertFullName.join(",");
          var queryInsertTeguranLisanId = `UPDATE ${endPeriodeDynamic}.${namaTable} SET audit_surat_id='${auditSuratIdString}', audit_surat_name='${fullNameString}' WHERE id='${id}'`;
          ...
          for (var j = 0; j < allemIdUser.length; j++) {
            ...
            insertIds.push(insertSuratPeringatan.insertId);
            insertFullName.push(fullNameUser);
            console.log("ini listkonsekuensi", listKonsekuensi);
            const konsekuensiArray = listKonsekuensi.map((k) => k.konsekuensi);
            for (var i = 0; i < konsekuensiArray.length; i++) {
