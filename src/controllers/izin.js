...
module.exports = {
  ...
  async store(req,res){
    ...
    console.log(leaveType)
            ...
            //jika fullday
            if (leaveType=="FullDay" || leaveType=="Full Day" || leaveType=="Full day"){   
                  ...
                  connection.beginTransaction((err) => {
                    ...
                    for(var i=0;i<dates.length;i++){
                       ...
                       const databasePeriode = `${database}_hrm${year}${month}`;
                        ...
                        connection.query( `SELECT * FROM ${databasePeriode}.emp_leave WHERE em_id='${emId}' AND date_selected LIKE '%${d}%' AND status_transaksi=1 AND (leave_status='Pending' OR leave_status='Approve' OR leave_status='Approve1' OR leave_status='Approve2')`, (err, results) => {
                          if (err) {
                        ...
                        console.log(`Masuk 1 ${records.length}`)
                          if (records.length>0){
                            error=true;
                                ...
                                pesan= `Kamu mempunyai pengajuan ${results[0].ajuan==1?"Cuti":results[0].ajuan==2?"Sakit":results[0].ajuan==3?"Izin":"Dinas Luar"} dengan nomor ${results[0].nomor_ajuan}  pada tanggal ${d} status ${results[0].leave_status}`
                          }

                          connection.query( `SELECT * FROM ${databasePeriode}.emp_labor WHERE em_id='${emId}' AND ajuan='2' AND atten_date='${d}' AND status_transaksi=1 AND (status='Pending' OR status='Approve' OR status='Approve1' OR status='Approve2')`, (err, results) => {
                            if (err) {
                              console.error('Error executing SELECT statement:', err);
                              connection.rollback(() => {
                                error=true;
                            ...
                            if (results.length>0){
                              ...
                              connection.query( `SELECT * FROM ${databasePeriode}.attendance WHERE em_id='${emId}' AND atten_date='${d}'`, (err, results) => {
                                if (err) {
                                  error=true;
                                  console.error('Error executing SELECT statement:', err);
                                  connection.rollback(() => {
                                ...
                                if (results.length>0){
                                  ...
                                  if (error==false){
                                    ...
                                    connection.query( `SELECT * FROM ${namaDatabaseDynamic}.emp_leave WHERE nomor_ajuan='${req.body.nomor_ajuan}'`, (err, results) => {
                                      if (err) {
                                        console.error('Error executing SELECT statement:', err);
                                        connection.rollback(() => {
                                          connection.end();
                                ...
                                //  proses memasukan data 
                                 ...
                                 connection.query( `INSERT INTO ${namaDatabaseDynamic}.emp_leave SET ?`,[insertData], (err, results) => {
                                  if (err) {
                                    console.error('Error executing SELECT statement:', err);
                                    connection.rollback(() => {
                                      connection.end();
                                  ...
                                  connection.query( `INSERT INTO ${namaDatabaseDynamic}.logs_actvity SET ?`,[dataInsertLog], (err, results) => {
                                      ...
                                      connection.query( `SELECT * FROM ${databaseMaster}.employee WHERE em_id='${emId}'`, (err, employee) => {
                                        ...
                                        connection.query(`SELECT * FROM ${namaDatabaseDynamic}.emp_leave WHERE nomor_ajuan='${req.body.nomor_ajuan}'`, (err, transaksi) => {
                                          if (err) {
                                            console.error('Error executing SELECT statement:', err);
                                            connection.rollback(() => {
                                              connection.end();
                                          ...
                                          connection.query(`SELECT * FROM sysdata WHERE kode='032'`, (err, sysdata) => {
                                            if (err) {
                                              console.error('Error executing SELECT statement:', err);
                                              connection.rollback(() => {
                                                connection.end();
                          ...
                          utility.insertNotifikasi(employee[0].em_report_to,'Approval Izin','Izin',employee[0].em_id,transaksi[0].id,transaksi[0].nomor_ajuan,employee[0].full_name,namaDatabaseDynamic,databaseMaster);
                          utility.insertNotifikasi(sysdata[0].name,'Pengajuan SakiT','Izin',employee[0].em_id,transaksi[0].id,transaksi[0].nomor_ajuan,employee[0].full_name,namaDatabaseDynamic,databaseMaster);
                    ...
                    if (records.length>0){
                      error=true;
                          ...
                          return res.status(400).json({
                              status: true,
                              message: `Kamu mempunyai pengajuan ${results[0].ajuan==1?"Cuti":results[0].ajuan==2?"Sakit":results[0].ajuan==3?"Izin":"Dinas Luar"} dengan nomor ${results[0].nomor_ajuan}  pada tanggal ${d} status ${results[0].leave_status}`,
                              data:[]
  ...
  async historyIzin(req, res) {
   ...
   var query=''
     ...
     if (i==0){
      ...
      query=query +`UNION ALL  SELECT   a.id as idd,a.*, b.name, b.category, b.leave_day, b.status, b.cut_leave,b.input_time,b.backdate as back_date FROM ${finalDatabase}.emp_leave a JOIN ${database}_hrm.leave_types b ON a.typeid=b.id WHERE a.em_id='${emId}' AND a.ajuan IN ('2','3','4') AND a.status_transaksi='1'  `
