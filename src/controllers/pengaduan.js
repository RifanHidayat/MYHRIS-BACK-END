        ...
        connection.beginTransaction((err) => {
          ...
          connection.query(query, [dataInsert], (err, results) => {
            ...
            for (var i = 0; i < details.length; i++) {
              ...
              connection.query(
                `INSERT INTO ${tableDetail} (nama_kerusakan,jumlah_kerusakan,lokasi_kerusakan,pengaduan_kerusakan_id,alasan_kerusakan)  
                            VALUES ('${details[i]["nama_kerusakan"]}','${details[i]["jumlah_kerusakan"]}','${details[i]["lokasi_kerusakan"]}','${records.insertId}','${details[i]["alasan_kerusakan"]}')`,
                details,
                (err, results) => {
                  if (err) {
                    console.error("Error executing SELECT statement:", err);
          ...
          connection.query(query2, (err, results) => {
            ...
            connection.query(query, [dataInsert], (err, results) => {
              ...
              for (var i = 0; i < details.length; i++) {
                ...
                connection.query(
                  `INSERT INTO ${tableDetail} (nama_kerusakan,jumlah_kerusakan,lokasi_kerusakan,pengaduan_kerusakan_id,alasan_kerusakan)  
                            VALUES ('${details[i]["nama_kerusakan"]}','${details[i]["jumlah_kerusakan"]}','${details[i]["lokasi_kerusakan"]}','${id}','${details[i]["alasan_kerusakan"]}')`,
                  details,
                  (err, results) => {
                    if (err) {
                      console.error("Error executing SELECT statement:", err);
