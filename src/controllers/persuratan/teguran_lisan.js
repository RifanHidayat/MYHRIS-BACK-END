    ...
    try {
      ...
      connection.connect((err) => {
        ...
        connection.beginTransaction((err) => {
          ...
          var queryTeguranLisan = `SELECT letter.name AS sp,employee.full_name AS nama,employee.job_title AS posisi, teguran_lisan.* 
          FROM teguran_lisan JOIN employee ON teguran_lisan.em_id=employee.em_id 
          LEFT JOIN letter ON letter.id=teguran_lisan.letter_id WHERE ${queryFilterStatus} AND approve_status='Approve' AND exp_date >= CURDATE() ORDER BY id DESC`;
          console.log(queryTeguranLisan);
          connection.query(queryTeguranLisan, (err, employee) => {
            if (err) {
              console.error("Error executing SELECT statement:", err);
            ...
            connection.commit((err) => {
              ...
              if (employee.length > 0) {
                ...
                var queryTeguranLisan = `SELECT letter.name AS sp,employee.full_name AS nama,employee.job_title AS posisi, teguran_lisan.* FROM teguran_lisan JOIN employee ON teguran_lisan.em_id=employee.em_id LEFT JOIN letter ON letter.id=teguran_lisan.letter_id 
                WHERE ${queryFilterStatus} AND teguran_lisan.status='Pending'  ORDER BY id DESC`;
                console.log(queryTeguranLisan);
                connection.query(queryTeguranLisan, (err, employee) => {
                  if (err) {
                    console.error("Error executing SELECT statement:", err);
