                ...
                if (err) {
                  ...
                  return;
                    ...
                    connection.commit((err) => {
                      ...
                      console.log("data login akhir e",records)
                      connection.end();
          ...
          connection.beginTransaction((err) => {
            ...
            //update
              ...
              // SELECT cust.email, cust_order.dbname, cust_order.pos, cust.password, company.name FROM cust_order JOIN cust ON cust.id=cust_order.cust_id JOIN company ON company.id=cust_order.company_id WHERE cust.email ='${email}'
                ...
                connection.query(`SELECT DISTINCT co.dbname,ess.email,CONCAT(c.name,' (',co.dbname,')') as name FROM cust_order co  JOIN company c ON c.id=co.company_id  JOIN ess ON ess.dbname=co.dbname WHERE ess.email='${email}' `, (err, results) => {
                  if (err) {
                    console.error('Error executing SELECT statement:', err);
                    connection.rollback(() => {
                      connection.end();
...
console.log('em_id 1',req.query)
    ...
    var script = `INSERT INTO notifikasi SET ?`;
