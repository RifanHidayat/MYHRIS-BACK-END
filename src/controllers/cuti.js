              ...
              connection.query(
                ...
                (err, data) => {
                  ...
                  if (data.length > 0) {
                    ...
                    /// jika suda ada data
                    var time1 = new Date(
                      `${data[0].atten_date}T${data[0].dari_jam}`
                    );
                    var time2 = new Date(
                      `${data[0].atten_date}T${data[0].sampai_jam}`
                    );
            ...
            (err, data) => {
              ...
              if (data.length > 0) {
                if (data[0].leave_type == "HALFDAY") {
                  ...
                  /// jika suda ada data
                  var time1 = new Date(
                    `${data[0].atten_date}T${data[0].time_plan}`
                  );
                  var time2 = new Date(
                    `${data[0].atten_date}T${data[0].time_plan_to}`
                  );
                  if (time1 > time2) {
                    time2.setDate(time2.getDate() + 1);
                  }
  ...
  async historyCuti(req, res) {
    ...
    for (var i = 0; i < datesplits.length; i++) {
      ...
      if (i == 0 || i == "0") {
        ...
        query =
          query +
          `UNION ALL SELECT a.id as idd, a.*, b.name, b.id as id_type FROM ${finalDatabase}.emp_leave a INNER JOIN ${database}_hrm.leave_types as b ON a.typeid=b.id WHERE a.em_id='${emId}'  
              AND a.status_transaksi='1'  AND b.cut_leave='1'  `;
      }
    }
