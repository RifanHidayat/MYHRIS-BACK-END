...
module.exports = {
  ...
  return tanggal;
    ...
    dateNow2() {
      ...
      let convertBulan =  month < 9 ? '0'+month : month;
    //  const tanggal = `${date} + '-' + ${convertBulan} + '-' + ${year};`
    const tanggal = `${year}-${convertBulan.toString().padStart(2, '0')  }-${date.toString().padStart(2, '0') }`  
    return tanggal;
    },
    addDate(date1,days) {
...
pushNotifikasiApproval(token,titile,message,url,emIdPengajuan,idx){
 ...
 async insertNotifikasi(emIds,title,url,emIdPengajuan,idx,nomorAjuan,namaPegajuan,databasePeriode,databseMaster){
          ...
          for (var i=0;i<listData.length;i++){
            ...
            if (listData[i]=='' || listData[i]==null){
              ...
              connection.query(    queryEmployee, (err, e) => {
               ...
               var deskripsi=`${namaPegajuan} mengajukan ${url=="TugasLuar"?"Tugas Luar":url} dengan nomor  ${nomorAjuan}`                                                            
 ...
 async insertNotifikasiGlobal(emIds,title,url,emIdPengajuan,idx,nomorAjuan,namaPegajuan,databasePeriode,databseMaster,description){


  function   pushNotifikasiApprovallobal(token,titile,message,url,emIdPengajuan,idx,nomorAjuan){
   ...
   console.log("emids ",emIds);
    ...
    async insertNotifikasiApproval(emIds,title,url,emIdPengajuan,idx,nomorAjuan,namaPegajuan,databasePeriode,databseMaster){
               ...
               for (var i=0;i<listData.length;i++){
                 ...
                 if (listData[i]=='' || listData[i]==null){
                   ...
                   connection.query(    queryEmployee, (err, e) => {
                     ...
                     var deskripsi=`${e[0].full_name} mengajukan ${url=="TugasLuar"?"Tugas Luar":url} dengan nomor  ${nomorAjuan}`                                                            
                     var query=`INSERT INTO ${databasePeriode}.notifikasi (em_id,title,deskripsi,url,atten_date,jam,status,view,em_id_pengajuan,idx)
                      VALUES ('${e[0].em_id}','${title}','${deskripsi}','${url}',CURDATE(),CURTIME(),2,0,'${emIdPengajuan}','${idx}')`
                      console.log(query)
                     connection.query(    query, (err, results) => {
  ...
  notifikasiChat(token,titile,message,emIdPengirim,emIdPenerima,emImage,jobTitle,){
    var token = `${token}`;
    var title = `${titile}`;
    var message = message;
