...
module.exports = {
  async show(req, res) {
    ...
    AND (   branch_id LIKE '%${branchId
      .toString()
      .padStart(2, "0")}%'  OR    branch_id LIKE '%${branchId}%' )
     `;
    try {
    ...
    var query = `SELECT ${table}.* FROM ${database}_hrm.${table} LEFT JOIN ${tableDetail} ON ${table}.id=${tableDetail}.peraturan_perusahaan_id AND ${tableDetail}.em_id='${emId}'  WHERE 
       status_transaksi='1' AND em_ids LIKE '%${emId}%' OR em_ids='' AND status='1' 
        AND (   branch_id LIKE '%${branchId
          .toString()
          .padStart(2, "0")}%'  OR    branch_id LIKE '%${branchId}%' )
    AND ${tableDetail}.peraturan_perusahaan_id IS AND   NULL ORDER BY ${table}.id ASC LIMIT 1`;
    ...
    try {
      ...
      connection.connect((err) => {
        ...
        connection.beginTransaction((err) => {
          ...
          console.log(
            "QUER ",
            `SELECT em_id FROM employee WHERE em_email='${email}' AND em_password='${password}'`
          );
          connection.query(
