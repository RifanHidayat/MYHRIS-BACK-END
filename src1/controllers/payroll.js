...
module.exports = {
  async bpjsKesehatan(req, res) {
    ...
    const [results] = await connection.query(
      ` SELECT * FROM emp_salary${tahun} WHERE initial IN ('A93','D51') AND em_id='${em_id}'  ORDER BY initial`
    );
    var tk = 0;
    var pt = 0;
  ...
  async bpjsKesehatanUpload(req, res) {
    ...
    var data = await connection.query(
      ` SELECT * FROM employee_file WHERE file_title LIKE '%${tahun}%' AND type='${type}' AND em_id='${em_id}'`
    );

    console.log(
      ` SELECT * FROM employee_file WHERE file_title LIKE '%${tahun}%' AND type='${type}'`
    );
  ...
  async bpjsKetagakerjaanUpload(req, res) {
    ...
    var data = await connection.query(
      `SELECT * FROM employee_file WHERE file_title LIKE '%${tahun}%' AND em_id='${em_id}' AND type='${type}'`
    );
