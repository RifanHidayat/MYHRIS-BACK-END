...
module.exports = {
  async store(req, res) {
    ...
    var durasiCicil =
      req.body.durasi_cicil == undefined ? 0 : req.body.durasi_cicil;
        ...
        connection.beginTransaction((err) => {
          ...
          connection.query(
            ...
            (err, employee) => {
              ...
              connection.query(
                `SELECT nomor_ajuan FROM emp_loan ORDER BY  nomor_ajuan DESC LIMIT 1 `,
                (err, data) => {
                  if (err) {
                    return connection.rollback(() => {
                      console.error("Error inserting into table2:", err);
                ...
                VALUES(CURDATE(),'${emId}', '${totalPinjaman}', '${ket}','${tanggalPinjaman}','${periode}','${nomorDefaut}',1,'Belum Lunas','${employee[0].branch_id}','${durasiCicil}','Pending','${durasiCicil}')`,
                    (err, results) => {
                      if (err) {
                      ...
                      connection.query(
                        `SELECT * FROM emp_loan WHERE nomor_ajuan='${nomorDefaut}'`,
                        (err, transaksi) => {
                          if (err) {
                            console.error(
                          ...
                          connection.query(
                            ...
                            (err, employee) => {
                              ...
                              var array = transaksi[0].tgl_ajuan
                                .toString()
                                .split("-");
                              ...
                              connection.query(
                                `SELECT * FROM sysdata WHERE kode='019'`,
                                (err, sysdata) => {
                                  if (err) {
                                    console.error(
                                      "Error executing SELECT statement:",
                                  ...
                                  var detailCicilan = req.body.cicilan;
                                  {
                                    for (
                                      var i = 0;
                                      i < detailCicilan.length;
                                      i++
                                    ) {
                                      var array =
                                        detailCicilan[i]["periode"].split("-");
                                      ...
                                      connection.query(
                                        `INSERT INTO emp_loan_installment(loan_id,description,ins_amount,status_payment,periode) VALUE ('${transaksi[0].id}','${transaksi[0].description}','${amount}','Belum Lunas','${periodeCicilan}')`,
                                        (err, results) => {
                                          if (err) {
                                            console.error(
                                              "Error executing SELECT statement:",
      ...
      connection.connect((err) => {
        ...
        connection.beginTransaction((err) => {
          ...
          connection.query(`${q}`, (err, results) => {
            ...
            connection.query(
              `DELETE FROM emp_loan_installment WHERE loan_id='${id}'`,
              (err, results) => {
                if (err) {
                  console.error("Error executing SELECT statement:", err);
                  connection.rollback(() => {
                ...
                var detailCicilan = req.body.cicilan;
                {
                  for (var i = 0; i < detailCicilan.length; i++) {
                    ...
                    periodeCicilan = `${year}-${motnh1}`;
                    connection.query(
                      `INSERT INTO emp_loan_installment(loan_id,description,ins_amount,status_payment,periode) VALUE ('${id}','${ket}','${amount}','Belum Lunas','${periodeCicilan}')`,
                      (err, results) => {
                        if (err) {
                          console.error(
                            "Error executing SELECT statement:",
    ...
    try {
      ...
      connection.connect((err) => {
        ...
        connection.beginTransaction((err) => {
          ...
          connection.query(
            `UPDATE emp_loan SET status_transaksi=0 WHERE id='${id}'  `,
            (err, results) => {
              if (err) {
                console.error("Error executing SELECT statement:", err);
                connection.rollback(() => {
    ...
    try {
      ...
      connection.connect((err) => {
        ...
        connection.beginTransaction((err) => {
          ...
          connection.query(
            `SELECT approve_by, emp_loan.jml_cicil as durasi_cicil,emp_loan.id, status, nomor_ajuan as nomor,tgl_ajuan as tanggal_ajuan,em_id,description,total_loan,periode_mulai_cicil as periode,tgl_mulai_cicil as tanggal_pinjaman
                 FROM emp_loan WHERE status_transaksi=1 AND em_id='${emId}'`,
            (err, results) => {
              if (err) {
                console.error("Error executing SELECT statement:", err);
                connection.rollback(() => {
              ...
              connection.query(
                `SELECT nomor_ajuan,emp_loan.status FROM emp_loan LEFT JOIN emp_loan_installment ON emp_loan.id=emp_loan_installment.loan_id WHERE emp_loan.status_transaksi=1 AND 
                ...
                (err, loan) => {
                  ...
                  if (loan.length > 0) {
                    if (loan[0]["status"] == "Pending") {
                      description =
                        "Anda tidak bisa mengajukan kasbon, karna anda mempuyai pengajuan dengan status pending";
                    }
    ...
    try {
      ...
      connection.connect((err) => {
        ...
        connection.beginTransaction((err) => {
          ...
          connection.query(
            `SELECT nomor_ajuan as nomor,tgl_ajuan as tanggal_ajuan,em_id,description,total_loan,periode_mulai_cicil as periode,tgl_ajuan as tanggal_pinjaman
                 FROM emp_loan`,
            (err, results) => {
              if (err) {
                console.error("Error executing SELECT statement:", err);
  ...
  async approval(req, res) {
    ...
    const namaDatabaseDynamic = `${database}_hrm${year
      .toString()
      .substring(year.toString().length - 2)}${month
      .toString()
      .padStart(2, "0")}`;
          ...
          connection.query(
            ...
            (err, employee) => {
              ...
              connection.query(query, (err, results) => {
                ...
                connection.query(
                  `SELECT * FROM emp_loan WHERE nomor_ajuan='${nomor}'`,
                  (err, results) => {
                    if (err) {
                      console.error("Error executing SELECT statement:", err);
                      connection.rollback(() => {
                    ...
                    var amount =
                      parseFloat(results[0].total_loan) /
                      parseFloat(results[0].durasi_cicil);
                    ...
                    connection.query(
                      ...
                      (err, sysdata) => {
                        ...
                        connection.query(
                          `SELECT * FROM employee WHERE em_id='${results[0].em_id}'`,
                          (err, user) => {
                            if (err) {
                              console.error(
                                "Error executing SELECT statement:",
