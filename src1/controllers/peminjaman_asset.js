        ...
        connection.beginTransaction((err) => {
          ...
          connection.query(query, (err, transaksi) => {
            ...
            connection.query(query, [dataInsert], (err, results) => {
              ...
              connection.query(
                `DELETE  FROM ${tableDetail} WHERE nomor_ajuan='${transaksi[0].nomor_ajuan}'`,
                (err, results) => {
                  if (err) {
                    console.error("Error executing SELECT statement:", err);
                    connection.rollback(() => {
                  ...
                  for (var i = 0; i < details.length; i++) {
                    ...
                    connection.query(
                      `INSERT INTO ${tableDetail} (trx_id,nomor_ajuan,tgl_ajuan,em_id,qty,remark,nourut,assets_id,tgl_pengembalian_ekstimasi) 
                        VALUES ('${id}','${transaksi[0].nomor_ajuan}','${tanggalPinjam}','${emId}','${qty}','${remark}','${noUrut}','${assetId}','${tglPengembalian}') `,
                      (err, results) => {
                        if (err) {
                          console.error(
    ...
    try {
      ...
      connection.connect((err) => {
        ...
        connection.beginTransaction((err) => {
          ...
          connection.query(
            ` SELECT nomor_ajuan FROM  ${table} ORDER BY nomor_ajuan DESC `,
            (err, results) => {
              if (err) {
                console.error("Error executing SELECT statement:", err);
                connection.rollback(() => {
              ...
              connection.query(query, [dataInsert], (err, results) => {
                ...
                for (var i = 0; i < details.length; i++) {
                  ...
                  var assetId = details[i]["assets_id"];
                  var qty =
                    details[i]["qty"] == undefined || details[i]["qty"] == ""
                      ? "1"
                      : details[i]["qty"];
                  var remark = details[i]["keterangan"];
                  var tglPengembalian = details[i]["tanggal_pengembalian"];
                  ...
                  connection.query(
                    `INSERT INTO ${tableDetail} (trx_id,nomor_ajuan,tgl_ajuan,em_id,qty,remark,nourut,assets_id,tgl_pengembalian_ekstimasi) 
                            VALUES ('${results.insertId}','${nomorAjuan}','${tanggalPinjam}','${emId}','${qty}','${remark}','${noUrut}','${assetId}','${tglPengembalian}') `,
                    (err, results) => {
                      if (err) {
                        console.error("Error executing SELECT statement:", err);
                        connection.rollback(() => {
      ...
      connection.connect((err) => {
        ...
        connection.beginTransaction((err) => {
          ...
          connection.query(query, (err, results) => {
            ...
            connection.query(
              `SELECT * FROM sysdata WHERE kode='043'`,
              (err, sysdata) => {
                if (err) {
                  console.error("Error executing SELECT statement:", err);
                  connection.rollback(() => {
                ...
                if (sysdata.length > 0) {
                  if (sysdata[0].name == "" || sysdata[0].name == null) {
                    ...
                    results.forEach((item) => {
                      ...
                      var datePengembalian = new Date(
                        utility.addDate(
                          item["tgl_pengembalian"],
                          parseInt("" + sysdata[0].name)
                        )
                      );
